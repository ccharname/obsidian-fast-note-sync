...
- ⚡ **同期パイプライン高速化**:
    - アップロードのバッチとダウンロードのページが、サーバーの確認応答を1つずつ待つ方式からスライディングウィンドウ・パイプライン方式に変わり、大規模な Vault の同期に必要なラウンドトリップ回数を大幅に削減します。
    > ⚠️ **注記**: v2.3.0 以降が必要です。サーバー v3.6.0 以降と組み合わせると最も効果的です（社内ベンチマークでは RTT がボトルネックとなる環境でアップロード最大6.64倍、ダウンロード最大3.19倍の高速化を計測）。旧バージョンのサーバーでは自動的に従来の動作にフォールバックするため、強制的なアップグレードは不要です。
- 🖼️ **添付ファイルの完全サポート**:
