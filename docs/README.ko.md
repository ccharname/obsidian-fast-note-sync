...
- ⚡ **동기화 파이프라인 가속**:
    - 업로드 배치와 다운로드 페이지가 서버 확인 응답을 하나씩 기다리는 방식 대신 슬라이딩 윈도우 파이프라인 방식으로 전송되어, 대형 저장소 동기화에 필요한 왕복 횟수를 크게 줄입니다.
    > ⚠️ **참고**: v2.3.0 이상이 필요합니다. 서버 v3.6.0 이상과 함께 사용할 때 가장 효과적입니다(내부 벤치마크에서 RTT 지연 환경 기준 업로드 최대 6.64배, 다운로드 최대 3.19배 속도 향상 측정). 구버전 서버에서는 자동으로 이전 동작으로 폴백되므로 강제 업그레이드가 필요하지 않습니다.
- 🖼️ **첨부 파일 완벽 지원**:
